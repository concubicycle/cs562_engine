...
{
	auto is_filter_callback = data->Flags & ImGuiInputTextFlags_CallbackCharFilter;
	return is_filter_callback && data->EventChar == '`' ? 1 : 0;
}

engineui::developer_console::developer_console(core::glfw_context& glfw) : view(glfw)
{
...
{
	ImGui::SetNextWindowSize({ (float)_glfw.width(), Height }, ImGuiCond_Always);
	ImGui::Begin("DevConsole");

	if (ImGui::IsWindowCollapsed())
		ImGui::SetWindowPos({ 0, (float)_glfw.height() - ImGui::GetWindowHeight() }, ImGuiCond_Always);
	else
		ImGui::SetWindowPos({ 0, (float)_glfw.height() - Height }, ImGuiCond_Always);
...
void engineui::developer_console::handle_command()
{
	if (strcmp(_input, "exit") == 0)
		glfwSetWindowShouldClose(_glfw.window(), true);	
